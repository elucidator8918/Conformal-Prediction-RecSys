...
class Autoencoder(nn.Module):
    ...
    def forward(self, x):
        encoded = torch.relu(self.encoder(x))  # This is the hidden layer (compressed representation)
...
class NeuCMF0iae(ContextRecommender):
    ...
    def __init__(self, config, dataset):
        ...
        input_size = (4 + self.n_contexts_dim) * self.mlp_embedding_size  # Assuming input features from embeddings
    ...
    def forward(self, user, item, context_situation_list):
        ...
        encoded_data, _ = self.autoencoder(torch.cat((user_mf_e, item_mf_e, user_mlp_e, item_mlp_e, context_situation_e), -1))
        if self.mf_train:
        ...
        if self.mf_train and self.mlp_train:
            ...
            output = self.predict_layer(torch.cat((mf_output, encoded_data), -1))
        elif self.mlp_train:
